...
def clean_llm_response(text):
    ...
    # إزالة التكرار المتتالي للأسطر (لمنع تكرار الإجابة)
    unique_lines = []
    for line in lines:
...
3. عند الاستفسار عن اسم شخص (مثل: "د. أحمد مرسي" أو "أ. ملاذ"):
   - ابحث عن هذا الاسم في **كافة أجزاء السياق المرجعي** المرفق.
   - ثم قم بجمع **كل** اللجان أو الوحدات الإدارية التي ورد فيها هذا الاسم واذكرها في قائمة واضحة.
   - لا تكتفِ بذكر لجنة واحدة فقط، بل اذكر جميع اللجان التي تم العثور عليها.
   - **مهم جداً:** لا تخلط بين اللجان الإدارية والوحدات الأكاديمية (المقررات). إذا لم يكن الاسم موجوداً في سياق المقررات، فقط اذكر اللجان.
4. **ممنوع تماماً:** أن تسأل الطالب عن الاسم الكامل إذا لم تجد معلومات. إذا لم تجد، أجب بالرد الافتراضي.
5. **ممنوع تماماً:** تكرار نفس الجملة أو الفقرة أكثر من مرة واحدة.
6. عند الاستفسار عن الأنشطة الطلابية لشهر معين، يجب تجميع كافة الأنشطة من جميع اللجان. ويمنع منعاً باتاً ذكر أي ميزانيات أو مبالغ مالية.
7. إذا لم تجد الإجابة في النص المرجعي، أجب فقط بـ: "عذراً، لا توجد معلومات صريحة في المصادر المرفقة. يُرجى مراجعة وحدة شؤون الطلبة."
        ...
        for model_name in models_to_try:
            try:
                completion = client.chat.completions.create(
                    model=model_name,
                    messages=[
                        {"role": "system", "content": "أنت مساعد دقيق يجيب باللغة العربية المباشرة. لا تكرر الإجابة. لا تسأل عن معلومات إضافية."},
